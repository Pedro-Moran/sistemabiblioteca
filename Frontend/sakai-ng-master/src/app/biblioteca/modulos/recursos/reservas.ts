                ...
                <p-iconfield>
                    ...
                    <ng-template pTemplate="body" let-objeto>
                        <tr>
                            ...
                            <td>
                                {{objeto.recurso.codigo}}
                            </td>
                            <td>
                                {{objeto.recurso.sede.descripcion}}
                            </td>
                            <td>
...
export class ReservasComponent implements OnInit {
    titulo: string = "Reservas";
    data: ClaseGeneral[] = [];
    modulo: string = "reservas";
    loading: boolean = true;
    objeto: Reservas = new Reservas();
    ...
    async listaReservas() {
        this.loading = true;
        this.reservasService.api_reservas_lista(this.modulo + '/listaPorSituacion/' + this.situacionFiltro.id)
            .subscribe(
                (result: any) => {
                    this.loading = false;
                    if (result.status == "0") {
